...
# Downloads, verifies SHA-256 (BEFORE extract), extracts, normalizes, launches

# --------------------------
# Config
# --------------------------
$ZipUrl         = 'https://github.com/dmooney-cs/dev01/raw/refs/heads/main/prod-01-01.zip'
$ExpectedSHA256 = 'd8b3055ae1a1bb8ce2c0604ae8962dd108164ac5f9b9b24db1cfc0d795046db8'  # 64 hex chars
$ExtractPath    = 'C:\CS-Toolbox-TEMP'
$DestRoot       = Join-Path $ExtractPath 'prod-01-01'
$ZipPath        = Join-Path $ExtractPath 'prod-01-01.zip'   # keep ZIP alongside extracted folder
$Launcher       = Join-Path $DestRoot 'CS-Toolbox-Launcher.ps1'

# Quieter progress for Invoke-WebRequest
...
# --------------------------
# Clean dest folder (avoid stale files)
# --------------------------
...
function Get-ZipSHA256([string]$Path) {
  try {
    ...
    # .NET fallback (restricted shells)
...
# Handles:
#  - ZIP already contains prod-01-01/
#  - Single nested folder with contents
#  - Mixed content at ExtractPath
# --------------------------
